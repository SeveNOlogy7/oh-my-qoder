...
Set up OMQ notification integrations so you're alerted when sessions end, need input, or complete background tasks.
...
Set up Telegram notifications so OMQ can message you when sessions end, need input, or complete background tasks.
...
2. Send /newbot
3. Choose a name (e.g., "My OMQ Notifier")
4. Choose a username (e.g., "my_omq_bot")
...
1. **Session end (Recommended)** - When a Qoder CLI session finishes
2. **Input needed** - When Qoder CLI is waiting for your response (great for long-running tasks)
...
RESPONSE=$(curl -s -w "\n%{http_code}" \
  ...
  -d "text=OMQ test notification - Telegram is configured!")
...
Config saved to: ~/.qoder/.omq-config.json

You can also set these via environment variables:
  OMQ_TELEGRAM_BOT_TOKEN=123456789:ABCdefGHI...
  OMQ_TELEGRAM_CHAT_ID=123456789

To reconfigure: /oh-my-qoder:configure-notifications telegram
To configure Discord: /oh-my-qoder:configure-notifications discord
To configure Slack: /oh-my-qoder:configure-notifications slack
```
...
```bash
export OMQ_TELEGRAM_BOT_TOKEN="123456789:ABCdefGHIjklMNOpqrsTUVwxyz"
export OMQ_TELEGRAM_CHAT_ID="123456789"
```

Env vars are auto-detected by the notification system without needing `.omq-config.json`.

---

## Discord Setup

Set up Discord notifications so OMQ can ping you when sessions end, need input, or complete background tasks.
...
**Options:**
1. **OMQ (default)** - Display as "OMQ"
2. **Qoder CLI** - Display as "Qoder CLI"
...
Build the notifications object with the collected values and merge into `.omq-config.json` using jq:
...
curl -s -o /dev/null -w "%{http_code}" \
  ...
  -d "{\"content\": \"${MENTION:+$MENTION\\n}OMQ test notification - Discord is configured!\"}" \
  "$WEBHOOK_URL"
```
...
You can also set these via environment variables:
  OMQ_DISCORD_WEBHOOK_URL=https://discord.com/api/webhooks/...
  OMQ_DISCORD_MENTION=<@1465264645320474637>

To reconfigure: /oh-my-qoder:configure-notifications discord
To configure Telegram: /oh-my-qoder:configure-notifications telegram
To configure Slack: /oh-my-qoder:configure-notifications slack
```
...
**Webhook method:**
```bash
export OMQ_DISCORD_WEBHOOK_URL="https://discord.com/api/webhooks/..."
export OMQ_DISCORD_MENTION="<@1465264645320474637>"  # optional
```

**Bot API method:**
```bash
export OMQ_DISCORD_NOTIFIER_BOT_TOKEN="your-bot-token"
export OMQ_DISCORD_NOTIFIER_CHANNEL="your-channel-id"
export OMQ_DISCORD_MENTION="<@1465264645320474637>"  # optional
```
...
Set up Slack notifications so OMQ can message you when sessions end, need input, or complete background tasks.
...
3. Name your app (e.g., "OMQ Notifier") and select your workspace
...
curl -s -o /dev/null -w "%{http_code}" \
  -H "Content-Type: application/json" \
  -d "{\"text\": \"${MENTION_PREFIX}OMQ test notification - Slack is configured!\"}" \
  "$WEBHOOK_URL"
```
...
You can also set these via environment variables:
  OMQ_SLACK_WEBHOOK_URL=https://hooks.slack.com/services/...
  OMQ_SLACK_MENTION=<@U1234567890>

To reconfigure: /oh-my-qoder:configure-notifications slack
To configure Discord: /oh-my-qoder:configure-notifications discord
To configure Telegram: /oh-my-qoder:configure-notifications telegram
```
...
```bash
export OMQ_SLACK_WEBHOOK_URL="https://hooks.slack.com/services/T00/B00/xxx"
export OMQ_SLACK_MENTION="<@U1234567890>"  # optional
```
...
- `omq --telegram` — Activates Telegram notifications (sets `OMQ_TELEGRAM=1`)
- `omq --discord` — Activates Discord notifications (sets `OMQ_DISCORD=1`)
- `omq --slack` — Activates Slack notifications (sets `OMQ_SLACK=1`)
- `omq --webhook` — Activates webhook notifications (sets `OMQ_WEBHOOK=1`)
- `omq --openclaw` — Activates OpenClaw gateway integration (sets `OMQ_OPENCLAW=1`)
...
Customize notification messages per event and per platform using `omq_config.hook.json`.
...
Config file: ~/.qoder/omq_config.hook.json
```
...
**Options:**
1. **session-end** - When a Qoder CLI session finishes (most common)
2. **ask-user-question** - When Qoder CLI is waiting for input
3. **session-idle** - When Qoder CLI finishes and waits for input
...
RAW FIELDS:
  ...
  {{activeMode}}     - Active OMQ mode name
...
- `/oh-my-qoder:configure-openclaw` — Configure OpenClaw gateway integration
...
If `~/.qoder/omq_config.openclaw.json` exists, detect and offer migration:
...
If migrate:
- Read `omq_config.openclaw.json`
- Transform to custom integration format
- Save to `.omq-config.json`
- Backup legacy to `omq_config.openclaw.json.bak`
...
curl -X POST \
  ...
  -d '{"event":"test","instruction":"OMQ test notification","timestamp":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' \
  "$WEBHOOK_URL"
```
