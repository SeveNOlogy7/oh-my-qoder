...
Meta-skill for managing oh-my-qoder skills via CLI-like commands.
...
**Built-in skill note:** Built-in skills are bundled with oh-my-qoder and are discoverable/readable, but not removed or edited through `/skill remove` or `/skill edit`.
...
✓ Created skill at ~/.qoder/skills/omq-learned/custom-logger/SKILL.md
...
Assistant: Found skill 'old-logger' in user scope:
  ...
  Scope: user (~/.qoder/skills/omq-learned/)
...
```
Skill: custom-logger
Scope: user (~/.qoder/skills/omq-learned/custom-logger/)
Description: Enhanced logging with structured output
Triggers: log, logger, logging
Argument Hint: <level> [message]
Quality: 95% (if available)
Usage Count: 42 times (if available)
File Path: /home/user/.qoder/skills/omq-learned/custom-logger/SKILL.md
...
Description: Enhanced logging with structured output
Triggers: log, logger, logging
File: ~/.qoder/skills/omq-learned/custom-logger/SKILL.md
...
User: yes
Assistant: ✓ Copied 'error-handler' to .omq/skills/
...
echo ""
echo "=== PROJECT-LEVEL SKILLS (.omq/skills/) ==="
if [ -d ".omq/skills" ]; then
  PROJECT_COUNT=$(find ".omq/skills" -name "*.md" 2>/dev/null | wc -l)
  echo "Total skills: $PROJECT_COUNT"

  if [ $PROJECT_COUNT -gt 0 ]; then
    echo ""
    echo "Skills found:"
    find ".omq/skills" -name "*.md" -type f -exec sh -c '
      FILE="$1"
      NAME=$(grep -m1 "^name:" "$FILE" 2>/dev/null | sed "s/name: //")
      DESC=$(grep -m1 "^description:" "$FILE" 2>/dev/null | sed "s/description: //")
      MODIFIED=$(stat -c "%y" "$FILE" 2>/dev/null || stat -f "%Sm" "$FILE" 2>/dev/null)
...
Then ask for scope:
- **User-level** (~/.qoder/skills/omq-learned/) - Available across all projects
- **Project-level** (.omq/skills/) - Only for this project
...
- `/oh-my-qoder:skill list` - Show detailed skill inventory
- `/oh-my-qoder:skill add` - Start skill creation (invoke skillify)
- `/oh-my-qoder:skill scan` - Scan both skill directories
...
**Version Control**: Project-level skills (`.omq/skills/`) are intended to be committed with your code so the whole team benefits. In linked worktrees, uncommitted skills remain local to that worktree and disappear if it is removed.
...
- `/oh-my-qoder:skillify` - Extract a skill from current conversation (`/oh-my-qoder:learner` is a deprecated alias)
- `/oh-my-qoder:note` - Save quick notes (less formal than skills)
- `/oh-my-qoder:deepinit` - Generate AGENTS.md codebase hierarchy
...
```
> /oh-my-qoder:skill list

Checking skill directories...
✓ User skills directory exists: ~/.qoder/skills/omq-learned/
✓ Project skills directory exists: .omq/skills/
...
- Run `/oh-my-qoder:skill list` periodically to review your skill library
