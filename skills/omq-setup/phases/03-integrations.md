...
```bash
grep -q "oh-my-qoder" "${QODER_CONFIG_DIR:-${QODERCN_CONFIG_DIR:?not set - run this inside a Qoder session}}/settings.json" && echo "Plugin verified" || echo "Plugin NOT found - run: qodercli plugins install oh-my-qoder"
```

## Step 3.2: Offer MCP Server Configuration

MCP servers extend Qoder CLI with additional tools (web search, GitHub, etc.).
...
Agent teams are an experimental Qoder CLI feature that lets you spawn N coordinated agents working on a shared task list with inter-agent messaging. **Teams are disabled by default** and require enabling via `settings.json`.

Reference: https://code.claude.com/docs/en/agent-teams

Use AskUserQuestion:

**Question:** "Would you like to enable agent teams? Teams let you spawn coordinated agents (e.g., `/team 3:executor 'fix all errors'`). This is an experimental Qoder CLI feature."
...
**CRITICAL**: Agent teams require `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS` to be set in `~/.qoder/settings.json`. This must be done carefully to preserve existing user settings.

First, read the current settings.json:

```bash
SETTINGS_FILE="${QODER_CONFIG_DIR:-${QODERCN_CONFIG_DIR:?not set - run this inside a Qoder session}}/settings.json"
...
Then use the Read tool to read `$QODER_CONFIG_DIR/settings.json` (if it exists). Use the Edit tool to merge the teams configuration while preserving ALL existing settings.
...
**Note:** Teammates do not have a separate model default. Each teammate is a full Qoder CLI session that inherits your configured model. Subagents spawned by teammates can use any model tier.
...
Skip this step. Agent teams will remain disabled. User can enable later by adding to `~/.qoder/settings.json`:
```json
{
  "env": {
    "CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS": "1"
...
Or by running `/oh-my-qoder:omq-setup --force` and choosing to enable teams.

## Save Progress

```bash
CONFIG_TYPE=$(jq -r '.configType // "unknown"' ".omq/state/setup-state.json" 2>/dev/null || echo "unknown")
bash "${OMQ_SETUP_PLUGIN_ROOT:-${QODER_PLUGIN_ROOT}}/scripts/setup-progress.sh" save 6 "$CONFIG_TYPE"
```
