...
Teams use Qoder CLI native tools (TeamCreate/SendMessage/TaskCreate).
...
Spawn coordinated agents with shared task lists and real-time messaging:
- /oh-my-qoder:team 3:executor "fix all TypeScript errors"
- Uses Qoder CLI native tools (TeamCreate/SendMessage/TaskCreate)
...
```bash
mkdir -p .qoder/rules
cp "${OMQ_SETUP_PLUGIN_ROOT:-${QODER_PLUGIN_ROOT}}/templates/rules/"*.md .qoder/rules/
```
...
bash "${OMQ_SETUP_PLUGIN_ROOT:-${QODER_PLUGIN_ROOT}}/scripts/setup-progress.sh" complete "$OMQ_VERSION"
```
