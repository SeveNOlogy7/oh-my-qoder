---
name: team
description: N coordinated agents on shared task list using OMQ file-based team tools
...
Spawn N coordinated agents working on a shared task list using OMQ's self-contained file-based team tools (MCP). Team coordination is implemented entirely at the plugin layer via `team_*` MCP tools — no host CLI built-in team support required.

**Architecture:** Team lifecycle, task management, and inter-agent messaging use file-based IPC under `.omq/state/team/{teamName}/`. Member spawning uses Qoder CLI's built-in `Agent` tool. All coordination flows through these MCP tools:
- `team_create` / `team_delete` — lifecycle
- `team_task_create` / `team_task_list` / `team_task_get` / `team_task_update` — shared task list
- `team_claim_task` / `team_next_ready_task` / `team_transition_task` / `team_release_claim` — atomic task claim protocol
- `team_register_worker` / `team_update_heartbeat` — worker identity and liveness
- `team_send_message` / `team_broadcast` / `team_list_mailbox` / `team_mark_message_delivered` — messaging
- `team_request_shutdown` / `team_read_shutdown_ack` — graceful shutdown
- `team_status` — aggregate status view
- `team_reap` — reclaim stale tasks from dead workers
...
- **agent-type** - OMQ agent to spawn for the `team-exec` stage (e.g., executor, debugger, designer, codex, gemini). Optional; defaults to stage-aware routing. Use `codex` to spawn Codex CLI workers or `gemini` for Gemini CLI workers (requires respective CLIs installed). See Stage Agent Routing below.
...
User: "/team 3:executor fix all TypeScript errors"
      ...
      [TEAM ORCHESTRATOR (Lead)]
              |
              +-- team_create("fix-ts-errors")
              |       -> lead becomes team-lead@fix-ts-errors
              |
              +-- Analyze & decompose task into subtasks
              |       -> explore/architect produces subtask list
              |
              +-- team_task_create x N (one per subtask)
              |       -> tasks #1, #2, #3 with dependencies
              |
              +-- Agent(subagent_type="oh-my-qoder:executor", prompt="<worker loop prompt>") x 3
              |       -> each worker calls team_register_worker on startup
              |       -> each worker loops: team_update_heartbeat → team_next_ready_task → work → team_transition_task
              |
              +-- Monitor loop
              |       -> team_status polling for progress + heartbeats
              |       -> team_reap to reclaim stale tasks from dead workers
              |       -> team_send_message to unblock/coordinate
              |
              +-- Completion
                      -> team_request_shutdown to each worker
                      <- team_read_shutdown_ack confirms exit
                      -> team_delete("fix-ts-errors")
```

**Storage layout (managed by Qoder CLI):**
...
Team is the OMQ authority for parallel, staged execution. Use the deterministic conflict policies `refuse`, `adopt_existing`, and `artifact_only` rather than non-deterministic warning handling. If a task mentions Qoder CLI `/goal`, Ralph, UltraQA, or artifact-only Ultragoal, keep Team as the primary loop authority unless the leader explicitly hands off. Use `/goal` only as a documented native Qoder CLI handoff target or as visible evidence from the lead session; do not claim the `/goal` evaluator independently runs commands, reads files, or replaces `team-verify` / `team-fix`. Artifact-only Ultragoal references should be treated as durable goal ledger/checkpoint/evidence artifacts, not as worker execution by themselves.
...
- **team-exec**
  - Entry: `team_create`, `team_task_create`, assignment, and worker spawn are complete.
...
3. **On team cancellation, handoffs survive** in `.omq/handoffs/` for session resume. They are not deleted by `team_delete`.
...
- Extract **agent-type**, validate it maps to a known OMQ subagent
...
Call `team_create` with a slug derived from the task:
...
Write OMQ state using the `state_write` MCP tool for proper session-scoped persistence:
...
Call `team_task_create` for each subtask. Set dependencies with `team_task_update` using `addBlockedBy`.

```json
// team_task_create for subtask 1
{
...
For tasks with dependencies, use `team_task_update` after creation:
...
- Internal tasks appear in `team_task_list` output -- filter them when counting real tasks
...
1. **Inbound messages** -- Teammates send `team_send_message` to `team-lead` when they complete tasks or need help. These arrive automatically as new conversation turns (no polling needed).

2. **team_task_list polling** -- Periodically call `team_task_list` to check overall progress:
   ```
...
- **Reassign work:** If a teammate finishes early, use `team_task_update` to assign pending tasks to them and notify via `team_send_message`
...
On every stage transition, update OMQ state:
...
1. **Verify results** -- Check that all subtasks are marked `completed` via `team_task_list`
...
4. **Delete team** -- Call `team_delete` to clean up:
   ```json
   { "team_name": "fix-ts-errors" }
   ```
   Response:
   ...
   ```
5. **Clean OMQ state** -- Remove `.omq/state/team-state.json`
...
1. CLAIM: Call team_task_list to see your assigned tasks (owner = "{worker_name}").
   Pick the first task with status "pending" that is assigned to you.
   Call team_task_update to set status "in_progress":
...
4. REPORT: Notify the lead via team_send_message:
   {"type": "message", "recipient": "team-lead", "content": "Completed task #ID: <summary of what was done>", "summary": "Task #ID complete"}

5. NEXT: Check team_task_list for more assigned tasks. If you have more pending tasks, go to step 1.
...
Check team_task_list periodically to see if blockers have been resolved.
...
- ALWAYS report progress via team_send_message to "team-lead"
- Use team_send_message with type "message" only -- never "broadcast"
```
...
- `claude_worker`: Emphasize strict team_task_list/team_task_update/team_send_message loop and no orchestration commands.
...
**CRITICAL: Steps must execute in exact order. Never call team_delete before shutdown is confirmed.**

**Step 1: Verify completion**

```
Call team_task_list — verify all real tasks (non-internal) are completed or failed.
```
...
**Step 4: team_delete — only after ALL teammates confirmed or timed out**
...
Check for agent processes that survived team_delete:
...
**Shutdown sequence is BLOCKING:** Do not proceed to team_delete until all teammates have either:
...
| --------------- | ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `claude_worker` | Claude agent           | Full Qoder CLI tool access (Read/Write/Edit/Bash/Task). Best for tasks needing Claude's reasoning + iterative tool use.                                                                  |
...
- CLI workers operate via tmux, not Qoder CLI's tool system
- They cannot use team_task_list/team_task_update/team_send_message (no team awareness)
...
| Tasks needing team coordination  | Claude teammate                | Needs team_send_message for status updates                |
...
1. Spawn `Agent(subagent_type="oh-my-qoder:planner", ...)` with task description + codebase context
...
The lead can proactively ingest outbox messages from CLI workers using the outbox reader utilities, enabling event-driven monitoring without relying solely on `team_send_message` delivery.
...
This approach complements the existing `team_send_message`-based communication by providing a pull-based mechanism for MCP workers that cannot use Qoder CLI's team messaging tools.

## Error Handling

### Teammate Fails a Task

1. Teammate sends `team_send_message` to lead reporting the failure
2. Lead decides: retry (reassign same task to same or different worker) or skip
3. To reassign: `team_task_update` to set new owner, then `team_send_message` to the new owner

### Teammate Gets Stuck (No Messages)

1. Lead detects via `team_task_list` -- task stuck in `in_progress` for too long
2. Lead sends `team_send_message` to the teammate asking for status
3. If no response, consider the teammate dead
4. Reassign the task to another worker via `team_task_update`

### Dependency Blocked

1. If a blocking task fails, the lead must decide whether to:
   - Retry the blocker
   - Remove the dependency (`team_task_update` with modified blockedBy)
   - Skip the blocked task entirely
2. Communicate decisions to affected teammates via `team_send_message`
...
4. If needed, spawn a replacement worker with `Agent(subagent_type="oh-my-qoder:executor", prompt="<worker loop prompt>")`
...
| ----------------------- | ------------------------------------------------------------------ | -------------------------------------- |
| **Storage**             | JSON files in `~/.qoder/teams/` and `~/.qoder/tasks/`            | SQLite in `.omq/state/swarm.db`        |
| **Dependencies**        | `better-sqlite3` not needed                                        | Requires `better-sqlite3` npm package  |
| **Task claiming**       | `team_task_update(owner + in_progress)` -- lead pre-assigns              | SQLite IMMEDIATE transaction -- atomic |
| **Race conditions**     | Possible if two agents claim same task (mitigate by pre-assigning) | None (SQLite transactions)             |
| **Communication**       | `team_send_message` (DM, broadcast, shutdown)                            | None (fire-and-forget agents)          |
| **Task dependencies**   | Built-in `blocks` / `blockedBy` arrays                             | Not supported                          |
| **Heartbeat**           | Automatic idle notifications from Qoder CLI                      | Manual heartbeat table + polling       |
...
| **Progress visibility** | `team_task_list` shows live status with owner                            | SQL queries on tasks table             |
...
| **State cleanup**       | `team_delete` removes everything                                    | Manual `rm` of SQLite database         |

**When to use Team over Swarm:** Always prefer `/team` for new work. It uses Qoder CLI's built-in infrastructure, requires no external dependencies, supports inter-agent communication, and has task dependency management.
...
4. Call `team_delete` to remove team and task directories
...
If teammates are unresponsive, `team_delete` may fail. In that case, the cancel skill should wait briefly and retry, or inform the user to manually clean up `~/.qoder/teams/{team_name}/` and `~/.qoder/tasks/{team_name}/`.
...
Optional settings live in `.qoder/omq.jsonc` (project) or `~/.config/qoder-omq/config.jsonc` (user). Project values override user values; `OMQ_TEAM_ROLE_OVERRIDES` (env JSON) supersedes both.
...
- **ops.monitorIntervalMs** - How often to poll `team_task_list` (default: 30s)
- **ops.shutdownTimeoutMs** - How long to wait for shutdown responses (default: 15s)

> **Note:** Team members do not have a hardcoded model default. Each teammate is a separate Qoder CLI session that inherits the user's configured model. Since teammates can spawn their own subagents, the session model acts as the orchestration layer while subagents can use any model tier.
...
Precedence: `OMQ_TEAM_ROLE_OVERRIDES` > `.qoder/omq.jsonc` (project) > `~/.config/qoder-omq/config.jsonc` (user) > built-in defaults. Invalid JSON logs a warning and is ignored — env overrides are best-effort and never abort the run.

### Fallback when a CLI is missing

If the CLI for a configured provider is absent from `PATH` at spawn time, `buildLaunchArgs()` throws, the team lead emits a visible `team_send_message` warning, and the runtime falls back to a deterministic Claude assignment pre-computed by `buildResolvedRoutingSnapshot` (same tier + same agent, `provider: "claude"`). Fallback is loud by design — silent fallback is a test failure. Probe provider availability with `omq doctor --team-routing`.
...
1. `team_delete` handles all Qoder CLI state:
   - Removes `~/.qoder/teams/{team_name}/` (config)
   - Removes `~/.qoder/tasks/{team_name}/` (all task files + lock)
2. OMQ state cleanup via MCP tools:
   ```
   ...
   ```
3. Or run `/oh-my-qoder:cancel` which handles all cleanup automatically.

**IMPORTANT:** Call `team_delete` only AFTER all teammates have been shut down. `team_delete` will fail if active members (besides the lead) still exist in the config.
...
1. **Worktree creation**: Before spawning a worker, call `createWorkerWorktree(teamName, workerName, repoRoot)` to create an isolated worktree at `.omq/worktrees/{team}/{worker}` with branch `omq-team/{teamName}/{workerName}`.
...
1. **Internal tasks pollute team_task_list** -- When a teammate is spawned, the system auto-creates an internal task with `metadata._internal: true`. These appear in `team_task_list` output. Filter them when counting real task progress. The subject of an internal task is the teammate's name.

2. **No atomic claiming** -- Unlike SQLite swarm, there is no transactional guarantee on `team_task_update`. Two teammates could race to claim the same task. **Mitigation:** The lead should pre-assign owners via `team_task_update(taskId, owner)` before spawning teammates. Teammates should only work on tasks assigned to them.

3. **Task IDs are strings** -- IDs are auto-incrementing strings ("1", "2", "3"), not integers. Always pass string values to `taskId` fields.

4. **team_delete requires empty team** -- All teammates must be shut down before calling `team_delete`. The lead (the only remaining member) is excluded from this check.
