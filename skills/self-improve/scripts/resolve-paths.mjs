...
import { resolveOmqStateRoot } from '../../../scripts/lib/state-root.mjs';
...
export async function resolveSelfImprovePaths({ projectRoot = process.cwd(), topic = '', slug = '', sessionId = '' } = {}) {
  ...
  const omcRoot = await resolveOmqStateRoot(resolvedProjectRoot);
...
function printHelp() {
  process.stdout.write(
    [
      ...
      '- New runs default to .omq/self-improve/topics/<topic-slug>/',
      '- Pass --session-id to isolate parallel runs: .omq/self-improve/topics/<slug>/sessions/<sid>/',
      '- Legacy flat .omq/self-improve/ is preserved only when no topic/slug is supplied and a flat layout already exists',
      '',
    ].join('\n'),
  );
}
