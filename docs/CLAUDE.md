...
Direct writes OK for: `~/.claude/**`, `.omq/**`, `.claude/**`, `CLAUDE.md`, `AGENTS.md`.
</model_routing>
...
Session/worktree continuity: before editing after resume/compaction or inside a linked worktree, re-check `git status --short --branch`, current cwd, and relevant `.omq/state/` or `.omq/handoffs/` artifacts so work does not continue on the wrong branch or stale context.
...
<worktree_paths>
State root: `.omq/` by default, or `$OMQ_STATE_DIR/{project-id}/` when `OMQ_STATE_DIR` is set, or the parent `.omq/` when a `.omq-workspace` marker anchors a multi-repo workspace. Runtime state includes `.omq/state/`, `.omq/state/sessions/{sessionId}/`, `.omq/notepad.md`, `.omq/project-memory.json`, `.omq/plans/`, `.omq/research/`, `.omq/logs/`, `.omq/artifacts/`, `.omq/handoffs/`, and `.omq/ultragoal/`. These are ignored operational artifacts by default; `.omq/skills/**` is the intentional committable exception for project-scoped skills. In linked git worktrees, local `.omq/` state is removed with the worktree unless centralized via `OMQ_STATE_DIR`.
</worktree_paths>
