...
describe('hud omq state session scoping', () => {
  ...
  function createWorktree(): string {
    const dir = mkdtempSync(join(tmpdir(), 'omq-hud-state-'));
    tempDirs.push(dir);
    return dir;
  }
  ...
  it('keeps backward-compatible newest-session fallback when sessionId is omitted', () => {
    ...
    writeJson(join(omqRoot, 'state', 'sessions', 'session-a', 'ralph-state.json'), {
      active: true,
      iteration: 1,
      max_iterations: 5,
      current_story_id: 'story-a',
    }, older);
    writeJson(join(omqRoot, 'state', 'sessions', 'session-b', 'ralph-state.json'), {
      active: true,
      iteration: 4,
      max_iterations: 7,
  ...
  it('does not leak to other sessions or fallback files when a session-scoped file is missing', () => {
    ...
    writeJson(join(omqRoot, 'state', 'sessions', 'session-b', 'autopilot-state.json'), {
      active: true,
      phase: 'execution',
      iteration: 3,
      max_iterations: 10,
  ...
  it('applies session scoping to combined mode helpers', () => {
    ...
    writeJson(join(omqRoot, 'state', 'sessions', 'session-b', 'ultrawork-state.json'), {
      active: true,
