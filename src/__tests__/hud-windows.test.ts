...
describe('HUD Windows Compatibility', () => {
  describe('File Naming', () => {
    it('session-start.mjs should reference omq-hud.mjs', () => {
  ...
  describe('pathToFileURL for Dynamic Import', () => {
    ...
    it('shared HUD wrapper template uses pathToFileURL for OMQ_PLUGIN_ROOT path import', () => {
      // The OMQ_DEV/devPath branch was deleted; OMQ_PLUGIN_ROOT subsumes it.
      const templatePath = join(packageRoot, 'scripts', 'lib', 'hud-wrapper-template.txt');
      const content = readFileSync(templatePath, 'utf-8');
  ...
  describe('Cross-Platform Plugin Cache Path (#670)', () => {
    ...
    it('shared HUD wrapper template should respect QODER_CONFIG_DIR for plugin cache base', () => {
      const templatePath = join(packageRoot, 'scripts', 'lib', 'hud-wrapper-template.txt');
      const content = readFileSync(templatePath, 'utf-8');
    ...
    it('omq-doctor skill should use cross-platform Node.js commands', () => {
      const doctorPath = join(packageRoot, 'skills', 'omq-doctor', 'SKILL.md');
    ...
    it('usage-api should use path.join with separate segments', () => {
      ...
      expect(content).toContain("'plugins', 'oh-my-qoder', `.usage-cache-${source}.json`");
    });
  });
});
