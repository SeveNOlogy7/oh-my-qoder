...
describe('auto-update reconciliation', () => {
  ...
  it('syncs active plugin cache roots and logs when copy occurs', () => {
    ...
    const activeRoot = join(QODER_CONFIG_DIR, 'plugins', 'cache', 'omc', 'oh-my-claudecode', '4.1.5');
  ...
  it('fails reconciliation when active plugin cache repair reports validation errors', () => {
    const activeRoot = join(QODER_CONFIG_DIR, 'plugins', 'cache', 'omc', 'oh-my-claudecode', '4.14.1');
  ...
  it('dedupes plugin roots and ignores missing targets during sync', () => {
    const consoleLogSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
    const activeRoot = join(QODER_CONFIG_DIR, 'plugins', 'cache', 'omc', 'oh-my-claudecode', '4.1.5');
    const staleRoot = join(QODER_CONFIG_DIR, 'plugins', 'cache', 'omc', 'oh-my-claudecode', '4.1.4');
  ...
  it('allows standalone update when CLAUDE_PLUGIN_ROOT is inherited without an active Claude session', async () => {
    const pluginRoot = join(QODER_CONFIG_DIR, 'plugins', 'cache', 'omc', 'oh-my-claudecode', '4.1.5');
    const cacheRoot = join(QODER_CONFIG_DIR, 'plugins', 'cache', 'omc', 'oh-my-claudecode');
