 ...
 * + `OMQ_DEV=1` branch with a single `process.env.OMQ_PLUGIN_ROOT` resolution
 * step set automatically by `omq --plugin-dir <path>`.
 *
...
describe('HUD wrapper devPaths removal (binary-weaving-mountain)', () => {
  it('plugin-setup.mjs no longer contains an inline devPaths array', () => {
    ...
    expect(content).not.toContain('Workspace/oh-my-qoder/dist/hud/index.js');
  ...
  it('shared HUD wrapper template exists and uses OMQ_PLUGIN_ROOT', () => {
    ...
    expect(content).not.toContain('projects/oh-my-qoder');
  });
});
