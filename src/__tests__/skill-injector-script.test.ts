...
function isSkillDiscoveryPath(value) {
  ...
  return path === projectDir + '/.omq/skills' || path.includes('/.omq/skills/') || path.endsWith('/skills/omc-learned');
}
function isSkillStatePath(value) {
  const path = normalizedPath(value);
  return path.startsWith(projectDir + '/.omq/state/') && /skill-sessions-fallback(?:-state)?\\.json(?:\\.lock)?$/.test(path);
}
...
describe('skill-injector.mjs learned skill injection', () => {
  function writeLearnedSkill(projectDir: string) {
    mkdirSync(join(projectDir, '.omq', 'skills'), { recursive: true });
    writeFileSync(join(projectDir, '.omq', 'skills', 'release-notes.md'), `---
name: Release Notes
triggers:
  - release notes
---
