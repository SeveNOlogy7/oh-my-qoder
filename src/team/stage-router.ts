...
/**
 * Resolve a user-supplied `model` value for a Qwen worker.
 ...
 *   3. Orchestrator: provider is always pinned to 'qwen' (user cannot
 *      override, per Option E).
 *   4. Fill in defaults: provider='qwen', model=role-default-tier,
 ...
 *
 * Fallback is always a Qwen worker with the same model + agent as primary,
...
export function buildResolvedRoutingSnapshot(
  ...
  for (const role of CANONICAL_TEAM_ROLES) {
    ...
    // Fallback is always a Qwen worker. Its model is the Qwen-tier
