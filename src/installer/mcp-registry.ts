...
const MANAGED_START = '# BEGIN OMQ MANAGED MCP REGISTRY';
const MANAGED_END = '# END OMQ MANAGED MCP REGISTRY';
...
export function getUnifiedMcpRegistryPath(): string {
  return process.env.OMQ_MCP_REGISTRY_PATH?.trim() || getGlobalOmqConfigPath('mcp-registry.json');
}

function getUnifiedMcpRegistryStatePath(): string {
  return getGlobalOmqStatePath('mcp-registry-state.json');
}
...
function getUnifiedMcpRegistryPathCandidates(): string[] {
  ...
  return getGlobalOmqConfigCandidates('mcp-registry.json');
}

function getUnifiedMcpRegistryStatePathCandidates(): string[] {
  return getGlobalOmqStateCandidates('mcp-registry-state.json');
}
