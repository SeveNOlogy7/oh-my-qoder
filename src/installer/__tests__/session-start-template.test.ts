...
${'- preserve this startup guidance\n'.repeat(400)}
  ...
  it('compacts large OMQ AGENTS guidance and caps aggregate session context', () => {
...
describe('session-start template cwd validation (Wave B1)', () => {
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'omq-session-start-cwdval-'));
    fakeHome = join(tempDir, 'home');
    mkdirSync(fakeHome, { recursive: true });
    // A truly empty directory with no .omq-workspace or .git marker.
    // Keep it under fakeHome so validateCwd stops before ambient /tmp markers.
    emptyCwd = mkdtempSync(join(fakeHome, 'omq-empty-cwd-'));
  });
  ...
  it('emits warning to stderr and outputs no-op JSON when cwd has no .omq-workspace or .git marker', () => {
    ...
    // Must warn on stderr
    expect(stderr).toContain('[OMQ] session-start: refusing to use cwd');
    expect(stderr).toContain('no .omq-workspace or .git marker');
    ...
    expect(existsSync(join(emptyCwd, '.omq'))).toBe(false);
  });

  it('does NOT warn or skip when cwd contains a .git directory', () => {
    const gitProject = mkdtempSync(join(tmpdir(), 'omq-git-project-'));
    try {
      mkdirSync(join(gitProject, '.git'), { recursive: true });
      mkdirSync(join(gitProject, '.omq', 'state'), { recursive: true });
      ...
      expect(stderr).not.toContain('[OMQ] session-start: refusing to use cwd');
  ...
  it('does NOT warn or skip when cwd contains a .omq-workspace marker', () => {
    const wsProject = mkdtempSync(join(tmpdir(), 'omq-ws-project-'));
    try {
      writeFileSync(join(wsProject, '.omq-workspace'), '{}');
      mkdirSync(join(wsProject, '.omq', 'state'), { recursive: true });
  ...
  it('does NOT warn or skip when cwd is a SUBDIRECTORY of a .git repo (walks up)', () => {
    const gitProject = mkdtempSync(join(tmpdir(), 'omq-git-subdir-'));
    try {
      mkdirSync(join(gitProject, '.git'), { recursive: true });
      mkdirSync(join(gitProject, '.omq', 'state'), { recursive: true });
  ...
  it('does NOT warn or skip when cwd is a nested SUBDIR whose only anchor is a .omq-workspace at an ancestor (no .git anywhere)', () => {
    const wsRoot = mkdtempSync(join(tmpdir(), 'omq-ws-ancestor-'));
    try {
      // Place .omq-workspace at the ancestor root (no .git anywhere)
      writeFileSync(join(wsRoot, '.omq-workspace'), '{}');
      // cwd is a deeply nested subdirectory — no .git, no .omq-workspace at this level
      ...
      // validateCwd must walk up, find .omq-workspace at wsRoot, and accept
      expect(stderr).not.toContain('[OMQ] session-start: refusing to use cwd');
