...
describe('Contract 7: hook command portability (#2084, #2348)', () => {
  ...
  it('default config: commands use ${QODER_CONFIG_DIR:-$HOME/.qoder} pattern', async () => {
    delete process.env.QODER_CONFIG_DIR;
    vi.resetModules();
  ...
  it('no command contains a hardcoded home directory path', async () => {
    ...
    if (violations.length > 0) {
      ...
      expect.fail(
        ...
        `Hook commands must use $HOME or \${QODER_CONFIG_DIR:-$HOME/.qoder}, not resolved absolute home paths.`
      );
    }
  });
...
// ── Contract 8: Hook config commands reference known OMQ hook filenames ───────
