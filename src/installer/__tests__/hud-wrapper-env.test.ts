 ...
 * `OMQ_PLUGIN_ROOT` env-var step (highest priority).
 *
 * Plan: binary-weaving-mountain.
 *
 * Strategy: write the wrapper template (which is the same byte-for-byte string
 * the installer would write to <configDir>/hud/omq-hud.mjs) into a tmp dir,
 * stage a sibling `lib/config-dir.mjs` and a fake `dist/hud/index.js` marker,
 * then spawn `node <tmp>/omq-hud.mjs` with controlled env + stdin and assert
 ...
 * `<configDir>/plugins/cache/omq/oh-my-qoder/0.0.0-test-stub/dist/hud/index.js`.
...
function makeStubConfigDir(rootDir: string): string {
  ...
  const stubDir = join(
    configDir,
    'plugins', 'cache', 'omq', 'oh-my-qoder', CACHE_STUB_VERSION, 'dist', 'hud',
  );
...
function stage(): StagedWrapper {
  const dir = mkdtempSync(join(tmpdir(), 'omq-hud-wrapper-'));
  const libDir = join(dir, 'lib');
  mkdirSync(libDir, { recursive: true });

  // Stage a shim config-dir.mjs that re-exports getQoderConfigDir under the
  // name `getClaudeConfigDir` — matching what the wrapper template imports.
  // The real config-dir.mjs only exports `getQoderConfigDir`; the wrapper
  // template destructures `{ getClaudeConfigDir }`, so a straight copy would
  // crash with an undefined binding.
  writeFileSync(
    join(libDir, 'config-dir.mjs'),
    [
      `import { getQoderConfigDir } from ${JSON.stringify('file://' + CONFIG_DIR_MJS)};`,
  ...
  const wrapperPath = join(dir, 'omq-hud.mjs');
  ...
  writeFileSync(
    ...
    'process.stdout.write("FROM_OMQ_PLUGIN_ROOT\\n");\n',
    'utf8',
  );
...
describe('HUD wrapper — OMQ_PLUGIN_ROOT resolution', () => {
  ...
  it('case 1: OMQ_PLUGIN_ROOT set + dist/hud/index.js exists → loads from there', () => {
    const s = staged!;
    ...
    // Pin: step 1 (OMQ_PLUGIN_ROOT) fired, not the cache stub.
    expect(result.stdout).not.toContain(CACHE_STUB_MARKER);
  });

  it('case 2: OMQ_PLUGIN_ROOT set but dist/hud/index.js missing → falls through to cache step', () => {
    const s = staged!;
    ...
    expect(result.stdout).not.toContain('FROM_OMQ_PLUGIN_ROOT');
    expect(result.stdout).toContain(CACHE_STUB_MARKER);
    expect(result.stderr ?? '').not.toMatch(/Error|throw/i);
  });

  it('case 3: OMQ_PLUGIN_ROOT unset → cache step (step 2) fires', () => {
    const s = staged!;
    const isolatedConfig = makeStubConfigDir(s.dir);
    const result = runWrapper(s.wrapperPath, scrubbedEnv({
      QODER_CONFIG_DIR: isolatedConfig,
      // OMQ_PLUGIN_ROOT intentionally omitted
    }));
    expect(result.status).toBe(0);
  ...
  it('case 4: OMQ_PLUGIN_ROOT points at a non-existent dir → cache step fires', () => {
    const s = staged!;
    const isolatedConfig = makeStubConfigDir(s.dir);
  ...
  it('case 6: cache step is semver-aware — stable beats prerelease with same [M.m.p]', () => {
    ...
    const result = runWrapper(s.wrapperPath, scrubbedEnv({
      ...
      // OMQ_PLUGIN_ROOT intentionally omitted → cache step fires
    }));
    expect(result.status).toBe(0);
