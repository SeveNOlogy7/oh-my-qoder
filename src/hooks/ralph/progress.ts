...
/**
 * Get the path to progress.txt in .omq subdirectory
 */
export function getOmqProgressPath(directory: string): string {
  return join(getOmqRoot(directory), PROGRESS_FILENAME);
}

/**
 * Find progress.txt in a directory (checks both root and .omq)
 */
export function findProgressPath(directory: string): string | null {
  ...
  const omqPath = getOmqProgressPath(directory);
  if (existsSync(omqPath)) {
    return omqPath;
  }
...
export function initProgress(directory: string): boolean {
  ...
  const progressPath = getOmqProgressPath(directory);
...
export function appendProgress(
  ...
  if (!progressPath) {
    ...
    progressPath = getOmqProgressPath(directory);
  }
