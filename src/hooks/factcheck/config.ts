 ...
 *
 * Loads guard config from the OMQ config system with token expansion
...
/**
 * Expand ${HOME}, ${WORKSPACE}, and ${QODER_CONFIG_DIR} tokens in a string.
 */
export function expandTokens(value: string, workspace?: string): string {
  const home = homedir();
  const ws = workspace ?? process.env.OMQ_WORKSPACE ?? process.cwd();
  return value
    .replace(/\$\{HOME\}/g, home)
    .replace(/\$\{WORKSPACE\}/g, ws)
    .replace(/\$\{QODER_CONFIG_DIR\}/g, getQoderConfigDir());
}
...
/**
 * Load guards config from the OMQ config system.
 *
 * Reads the `guards` key from the merged OMQ config, deep-merges over
 * defaults, and expands ${HOME}/${WORKSPACE}/${QODER_CONFIG_DIR} tokens.
 */
