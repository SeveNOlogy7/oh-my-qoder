...
export async function executeSessionEndAction(name: SessionEndActionName, payload: SessionEndWorkerPayload, deadlineAt: number): Promise<void> {
  ...
  if (typeof routing?.notificationProfile === 'string') process.env.OMQ_NOTIFY_PROFILE = routing.notificationProfile;
  if (routing?.openClawEnabled === true) process.env.OMQ_OPENCLAW = '1';
