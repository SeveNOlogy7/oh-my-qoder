/**
 * OMQ HUD - State Readers
 *
 * Read ralph, ultrawork, and PRD state from existing OMQ files.
...
function resolveStatePath(directory: string, filename: string, sessionId?: string): string | null {
  const omqRoot = getOmqRoot(directory);

  if (sessionId) {
    const sessionPath = join(omqRoot, 'state', 'sessions', sessionId, filename);
  ...
  // Check standard path
  const newPath = join(omqRoot, 'state', filename);
  ...
  // Check legacy path
  const legacyPath = join(omqRoot, filename);
...
export function readPrdStateForHud(directory: string): PrdStateForHud | null {
  ...
  if (!existsSync(prdPath)) {
    // Check .omq
    prdPath = join(getOmqRoot(directory), 'prd.json');
...
/**
 * Check if any OMQ mode is currently active
 */
