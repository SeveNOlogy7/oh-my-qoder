 ...
 *
 * Affected files: skills/omq-setup/SKILL.md, skills/hud/SKILL.md
 */
...
describe('MINGW64 escape safety: no "!" in node -e inline scripts (issue #729)', () => {
  ...
  describe('skills/omq-setup (SKILL.md + phases)', () => {
    const setupDir = join(REPO_ROOT, 'skills', 'omq-setup');
    const filesToScan = [
    ...
    it('has no "!" in any node -e script body (MINGW64 safe)', () => {
      const violations = findBangViolations(allScripts, 'omq-setup/*');
      if (violations.length > 0) {
        expect.fail(
          'Found "!" in node -e scripts (breaks MINGW64/Git Bash):\n' +
          violations.map(v => `  • ${v}`).join('\n')
  ...
  describe('specific regressions (issue #729)', () => {
    ...
    it('hud SKILL.md cleanup step removes only the legacy HUD wrapper filename', () => {
      ...
      const cleanupLine = content
        .split('\n')
        .find(l => l.includes('Removed legacy omq-hud.js') && l.startsWith('node -e'));

      expect(cleanupLine).toBeDefined();
      expect(cleanupLine).toContain("t=p.join(d,'hud','omq-hud.js')");
      expect(cleanupLine).not.toContain("t=p.join(d,'hud','omq-hud.mjs')");
    });

    it("omq-setup version-detect script uses v==='' not !v", () => {
      const setupDir = join(REPO_ROOT, 'skills', 'omq-setup');
      const files = [
        join(setupDir, 'SKILL.md'),
        ...readdirSync(join(setupDir, 'phases')).map(f => join(setupDir, 'phases', f)),
    ...
    it('omq-setup extracts CLAUDE.md version from OMQ marker', () => {
      const setupDir = join(REPO_ROOT, 'skills', 'omq-setup');
      const files = [
        join(setupDir, 'SKILL.md'),
        ...readdirSync(join(setupDir, 'phases')).map(f => join(setupDir, 'phases', f)),
        join(REPO_ROOT, 'scripts', 'setup-agents-md.sh'),
      ].filter(f => f.endsWith('.md') || f.endsWith('.sh'));
      const combined = files.map(f => readFileSync(f, 'utf-8')).join('\n');
      expect(combined).toContain("grep -m1 'OMQ:VERSION:'");
      expect(combined).not.toContain('grep -m1 "^# oh-my-qoder"');
    });

    it('omq-setup SKILL.md explicitly tells the agent to execute immediately', () => {
      const content = readFileSync(
        join(REPO_ROOT, 'skills', 'omq-setup', 'SKILL.md'),
        'utf-8'
      );
    ...
    it('omq-setup phase 2 delegates HUD setup instead of inlining statusLine formatting', () => {
      const content = readFileSync(
