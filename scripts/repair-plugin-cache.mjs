#!/usr/bin/env node
/**
 * Repair OMQ plugin cache references after marketplace updates.
 *
 * Qoder CLI can keep a running session pointed at the previous plugin cache
...
function isValidOmcPluginRoot(root) {
  return existsSync(join(root, 'hooks', 'hooks.json'))
    || existsSync(join(root, 'skills', 'omq-setup', 'SKILL.md'))
...
export function repairPluginCacheReferences() {
  ...
  try {
    result.hooksPatched = patchHooksJsonForPlatform(latestRoot, process.env.OMQ_REPAIR_PLUGIN_CACHE_PLATFORM || process.platform);
  } catch (error) {
...
if (process.argv[1] && fileURLToPath(import.meta.url) === process.argv[1]) {
  ...
  for (const error of result.errors) {
    console.warn(`[OMQ] Plugin cache repair warning: ${error}`);
  }
  if (!result.latestRoot) {
    console.log('[OMQ] No OMQ plugin cache found (normal for new installs)');
  } else if (result.registryUpdated || result.hooksPatched || result.symlinked > 0) {
    console.log(`[OMQ] Repaired plugin cache references: active=${result.latestRoot}, symlinked=${result.symlinked}${result.hooksPatched ? ', hooks=platform' : ''}`);
  } else {
    console.log('[OMQ] Plugin cache references are current');
  }
}
