...
console.log('[OMQ] Running post-install setup...');

function checkRalphRubyDependency() {
  try {
    execFileSync('ruby', ['--version'], { stdio: 'ignore', timeout: 5000 });
    console.log('[OMQ] Ruby detected for Ralph workflows');
  } catch {
    console.log('[OMQ] Warning: Ruby was not found on PATH. Ralph workflows require Ruby and may fail until it is installed.');
    console.log('[OMQ] Ubuntu/Debian: sudo apt update && sudo apt install ruby-full');
    console.log('[OMQ] macOS: brew install ruby');
    console.log('[OMQ] After installing Ruby, restart Qoder CLI and rerun /oh-my-qoder:omq-setup if needed.');
  }
}
...
copyFileSync(join(__dirname, 'lib', 'hud-cache-wrapper.sh'), join(HUD_DIR, 'omq-hud-cache.sh'));
try { chmodSync(join(HUD_DIR, 'find-node.sh'), 0o755); } catch { /* Windows doesn't need this */ }
try { chmodSync(join(HUD_DIR, 'omq-hud-cache.sh'), 0o755); } catch { /* Windows doesn't need this */ }

// 2. Create HUD wrapper script
const hudScriptPath = join(HUD_DIR, 'omq-hud.mjs').replace(/\\/g, '/');
...
console.log('[OMQ] Installed HUD wrapper script');
...
try {
  ...
  const statusLineCommand = process.platform === 'win32'
    ...
    : `sh "${join(HUD_DIR, 'omq-hud-cache.sh').replace(/\\/g, "/")}" "${hudScriptPath.replace(/\\/g, "/")}"`;
  ...
  console.log('[OMQ] Configured HUD statusLine in settings.json');

  // Persist the node binary path to .omq-config.json for use by find-node.sh
  try {
    const configPath = join(CLAUDE_DIR, '.omq-config.json');
    let omqConfig = {};
    if (existsSync(configPath)) {
      omqConfig = JSON.parse(readFileSync(configPath, 'utf-8'));
    }
    if (nodeBin !== 'node') {
      omqConfig.nodeBinary = nodeBin;
      writeFileSync(configPath, JSON.stringify(omqConfig, null, 2));
      console.log(`[OMQ] Saved node binary path: ${nodeBin}`);
    }
  } catch (e) {
    console.log('[OMQ] Warning: Could not save node binary path (non-fatal):', e.message);
  }
} catch (e) {
  console.log('[OMQ] Warning: Could not configure settings.json:', e.message);
}

// Patch packaged plugin-cache hooks.json to keep plugin-provided hook commands
// safe for the platform that is installing the plugin cache. Qoder CLI's
...
try {
  ...
  if (hooksJsonPath && existsSync(hooksJsonPath)) {
    ...
    if (patched) {
      ...
      console.log(`[OMQ] Patched hooks.json to use ${platformLabel} hook commands`);
    }
  }
} catch (e) {
  console.log('[OMQ] Warning: Could not patch hooks.json:', e.message);
}
...
console.log('[OMQ] Setup complete! Restart Qoder CLI to activate HUD.');
